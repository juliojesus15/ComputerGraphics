...
public:
    ...
    std::vector< std::vector<unsigned int> > idx_objetos;
    ...
    //Animaciones
    void rotacion(glm::vec3 eje);
    void escala(bool flag);
...
Vitral::Vitral(float altura, float ancho, float x, float y, float z) {
    ...
    this->objetos.insert(objetos.end(), {
        backgroung, tallo, petalo, petalo_izq, petalo_der, hoja_izq, hoja_der
        });

    this->idx_objetos.insert(idx_objetos.end(), {
        idx_margen, idx_background, idx_tallo_margen, idx_tallo_background, idx_petalo, idx_petalo_izq, idx_petalo_der, idx_hoja_izq, idx_hoja_der
        });
...
void Vitral::rotacion(glm::vec3 eje) {
    glm::mat4 matriz_rotacion = glm::rotate(glm::mat4(1.0f), glm::radians(45.0f), eje);

    for (float i = 0; i < this->objetos.size(); i ++) {
        std::vector<float > tmp_objecto = objetos[i];
        for (float j = 0; j < tmp_objecto.size(); j += 3) {
            glm::vec4 vertice = glm::vec4(tmp_objecto[j], tmp_objecto[j + 1], 0.0f, 1.0f);
            glm::vec4 traslacion = matriz_rotacion * vertice;
            tmp_objecto[j] = traslacion.x;
            tmp_objecto[j + 1] = traslacion.y;
        }
        this->objetos[i] = tmp_objecto;
    }

    this->backgroung = objetos[0];
    this->tallo = objetos[1]; 
    this->petalo = objetos[2];
    this->petalo_izq = objetos[3];
    this->petalo_der = objetos[4];
    this->hoja_izq = objetos[5];
    this->hoja_der = objetos[6];
}


void Vitral::escala(bool flag) {
    float k = flag ? 1.2f : 0.8f;
    ...
    for (float i = 0; i < this->objetos.size(); i++) {
        ...
        for (float j = 0; j < tmp_objecto.size(); j += 3) {
            glm::vec4 vertice = glm::vec4(tmp_objecto[j], tmp_objecto[j + 1], 0.0f, 1.0f);
            glm::vec4 traslacion = matriz_escala * vertice;
            tmp_objecto[j] = traslacion.x;
            tmp_objecto[j + 1] = traslacion.y;
        }
        this->objetos[i] = tmp_objecto;
